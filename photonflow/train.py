...
class Trainer:
    ...
    def _build_model(self) -> PhotonFlowModel:
        ...
        return PhotonFlowModel(
            ...
            sigma_s=mcfg.get("sigma_s", 0.001),
            sigma_t=mcfg.get("sigma_t", 0.005),
            shot_signal_dependent=mcfg.get("shot_signal_dependent", True),
            monarch_init=mcfg.get("monarch_init", "random"),
            adaln_init_std=mcfg.get("adaln_init_std", 0.02),
            num_monarch_factors=mcfg.get("num_monarch_factors", 1),
            absorber_alpha=mcfg.get("absorber_alpha", 0.8),
            absorber_leaky_slope=mcfg.get("absorber_leaky_slope", 0.05),
            learnable_absorber_alpha=mcfg.get("learnable_absorber_alpha", False),
            mean_center_norm=mcfg.get("mean_center_norm", False),
            phase_noise_sigma=mcfg.get("phase_noise_sigma", 0.005),
            cumulative_loss_db_per_stage=mcfg.get("cumulative_loss_db_per_stage", 0.0003),
            seq_dim=mcfg.get("seq_dim", None),
            feat_dim=mcfg.get("feat_dim", None),
            cond_bias_hidden=mcfg.get("cond_bias_hidden", 0),
        )
