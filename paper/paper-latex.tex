...
re-ran both PhotonFlow-Strict ($S_\mathrm{scale\_4M}$,
baseline-parity, $4.87$\,M params, K2 recipe except
$\mathrm{warmup}{=}300/195\mathrm{K}$, cosine to zero over the full
horizon, kernel \texttt{photonflow-exp2-200k}) and the DiT-attention
baseline at $195\,000$ optimiser steps under matched seed and data
order (Table~\ref{tab:long}).

\begin{table}[tbp]
\caption{Long-horizon ($195\,000$ steps) eval and gap, both trained
under matched recipe, seed, and data order.}
\label{tab:long}
\centering\footnotesize
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Model} & \textbf{Eval @ $195$\,k} & \textbf{Gap}\\
\midrule
Baseline (DiT, $4.89$\,M)        & 0.0876 & $-$ \\
PhotonFlow-Strict ($4.87$\,M)    & 0.1595 & $+0.0719$ \\
\midrule
\multicolumn{1}{@{}l}{Both vs.\ their own $2$\,k checkpoint} & & \\
$\Delta$ Baseline ($2$\,k $\to 195$\,k)        & $-0.0858$ & $-$ \\
$\Delta$ PhotonFlow ($2$\,k $\to 195$\,k)      & $-0.0768$ & $-$ \\
\bottomrule
\end{tabular}
\end{table}
...
apples-to-apples gap at $195{,}000$ steps is $+0.0719$ -- modestly
\emph{wider} than the $+0.0629$ measured at $2$\,k for the same
$S_\mathrm{scale\_4M}$ configuration (Sec.~\ref{sec:exp-scale}).
Photon-native training is therefore not closing the gap by mere
optimisation budget; the residual $\sim\!7\%$ uniform-$t$ CFM
penalty appears to be an architectural ceiling of strict-photon-
native CFM at this $4.87$\,M parameter scale, separate from the
$30+$-variant additive ceiling broken in \S\ref{sec:exp-ceiling}.
We do \emph{not} promote a ``negative-gap'' claim against an
asymmetric short-baseline reference; the honest long-horizon read
is that PhotonFlow-Strict halves its $2$\,k gap-from-zero
($0.2363 \to 0.1595$) at the cost of $\sim\!100{\times}$ more
optimiser steps, while the baseline halves its own ($0.1734 \to
0.0876$) under the same compute.

Fig.~\ref{fig:samples} shows uncurated digits sampled from the
$195$\,k checkpoints (left: PhotonFlow-Strict via the
\texttt{OpticalSampler}, one comparator readout per sample, zero
electronic ops in the forward graph; right: baseline via the
20-step Euler ODE solver).  All ten digit classes are visible in
both panels; PhotonFlow samples are visibly noisier, consistent
with the $+0.0719$ eval gap.
...
$2$\,k optimiser steps; both models continue to descend at long
horizon (Sec.~\ref{sec:exp-samples}, $195$\,k matched run) but the
gap modestly widens from $+0.0629$ to $+0.0719$, indicating the
residual penalty is architectural rather than budgetary at this
parameter scale.  Closing this final gap likely requires either
parameter-scale increase (\S\ref{sec:exp-scale} shows monotone
gains up to $14.9$\,M) or an additional photon-native primitive --
softmax-attention's nearest published photonic analogue (the
all-optical attention chip~\cite{ning2024}) is the obvious next
target.  (b) CIFAR-10 FID,
...
zero forward-graph cost.  An apples-to-apples $195\,000$-step match-up at baseline parity
showed both models continue to train deeply (PhotonFlow
$0.2363\!\to\!0.1595$, baseline $0.1734\!\to\!0.0876$); the
apples-to-apples gap modestly widens from $+0.0629$ to $+0.0719$,
indicating that the residual penalty at the $4.87$\,M scale is
architectural rather than an optimisation-budget artefact.  We also report a clean negative result:
