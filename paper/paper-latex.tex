...
(Table~\ref{tab:scale}, Fig.~\ref{fig:scale}).  At \emph{exact
baseline parity} ($4.87$\,M, $N{=}5$) the gap is $+0.0629$ -- a
$\Delta\!=\!-0.0301$ absolute improvement over the additive
ceiling.  At $N{=}10$ ($9.4$\,M, $1.93\times$ baseline) the gap
falls to $+0.0418$; at $N{=}16$ ($14.9$\,M, $3.04\times$ baseline)
it falls to $+0.0365$.  The scaling is approximately
$\mathrm{gap}\!\propto\!1/\sqrt{P}$ within this range.

\begin{figure}[tbp]
\centering
\input{fig_scale}
\caption{Depth-scaling Pareto curve at the bs/lr-scaled recipe.
Three measured points ($N\!\in\!\{5,10,16\}$) plus the
$1.37\times$-baseline configuration of Table~\ref{tab:ceiling}
last row.  The dashed red line marks the $+0.05$ matched-recipe
target; the dotted vertical marks the $4.89$\,M baseline-parity
boundary.}
\label{fig:scale}
\end{figure}
